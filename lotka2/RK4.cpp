...
*		h      is the step size.
*/
void RK4::step(AbstractModell& model, Eigen::VectorXd& yi_plus_1,
    ...
    while (yi[0].size() > iter) {
        ...
        yi_new(iter) = yi[0](iter) + k1(iter) * 0.5;
        iter++;
    }
    ...
    iter = 0;
    while (yi[0].size() > iter) {
        yi_new(iter) = yi[0](iter) + k2(iter) * 0.5;
        iter++;
    }
